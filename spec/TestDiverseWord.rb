require_relative '../solution/DiverseWord'
...
RSpec.describe 'example test' do
    it "example1" do
        ...
        expect(checkResult(test, 6,1,1)).to eq true
    end
    it "example2" do
        test = DiverseWord.new(1,3,1).solution
        expect(checkResult(test, 1, 3,1)).to eq true
    end
    it "example3" do
        test = DiverseWord.new(0,1,8).solution
        expect(checkResult(test, 0, 1,8)).to eq true
    end
end
...
RSpec.describe 'Correctness tests' do
    it "super_small" do
        ...
        expect(checkResult(test, 1, 0, 1)).to eq true
    end
    it "two_zeroes" do
        test = DiverseWord.new(0, 0, 1).solution
        expect(checkResult(test, 0, 0, 1)).to eq true
    end
    it "no_a" do
        test = DiverseWord.new(0, 1, 5).solution
        expect(checkResult(test, 0, 1, 5)).to eq true
    end
    it "no_b" do
        test = DiverseWord.new(7, 0, 5).solution
        expect(checkResult(test, 7, 0, 5)).to eq true
    end
    it "no_c" do
        test = DiverseWord.new(7, 3, 0).solution
        expect(checkResult(test, 7, 3, 0)).to eq true
    end
    it "all_similar" do
        test = DiverseWord.new(5, 5, 6).solution
        expect(checkResult(test, 5, 5, 6)).to eq true
    end
    it "one_big" do
        test = DiverseWord.new(34, 5, 6).solution
        expect(checkResult(test, 34, 5, 6)).to eq true
    end
    it "two_big" do
        test = DiverseWord.new(34, 35, 6).solution
        expect(checkResult(test, 34, 35, 6)).to eq true
    end
    it "one_fixed_letter" do
        test = DiverseWord.new(12, 2, 3).solution
        expect(checkResult(test, 12, 2, 3)).to eq true
    end
    ...
    it "max" do
        ...
        expect(checkResult(test, 99, 99, 99)).to eq true
    end
end